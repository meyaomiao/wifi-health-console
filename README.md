...
<p align="center">
  <strong>当前源码版本 v0.4.2 · macOS 现有 v0.4.1 · Windows v0.4.2 Preview</strong>
  ·
  ...
  ·
  <a href="https://github.com/meyaomiao/wifi-health-console/releases/download/v0.4.2/WiFi-Health-Console-Setup-x64.exe">Windows x64 EXE</a>
  ·
  <a href="https://github.com/meyaomiao/wifi-health-console/releases/download/v0.4.2/WiFi-Health-Console-Setup-arm64.exe">Windows ARM64 EXE</a>
  ·
  <a href="https://github.com/meyaomiao/wifi-health-console/releases/tag/v0.4.2">Windows v0.4.2 发布说明</a>
</p>
...
> 当前源码版本为 v0.4.2；macOS 现有正式包为 v0.4.1，Windows 修复包为 v0.4.2 Preview。两个平台都可直接用图形界面安装与使用，不需要命令行。
...
| Windows | v0.4.2 Preview | Avalonia + Windows Native WLAN API | 预览版，提供 x64 / ARM64 安装器 |
...
1. 普通 Intel / AMD Windows 电脑下载 [x64 EXE 安装器](https://github.com/meyaomiao/wifi-health-console/releases/download/v0.4.2/WiFi-Health-Console-Setup-x64.exe)；Windows on ARM 设备下载 [ARM64 EXE 安装器](https://github.com/meyaomiao/wifi-health-console/releases/download/v0.4.2/WiFi-Health-Console-Setup-arm64.exe)。
...
当前 v0.4.2 Windows Preview 安装器尚未进行 Authenticode 签名，SmartScreen 可能显示“Windows 已保护你的电脑”或“未知发布者”。请确认安装包来自本项目的官方 [v0.4.2 Windows Release](https://github.com/meyaomiao/wifi-health-console/releases/tag/v0.4.2)，并优先对照发布页中的 `SHA256SUMS-windows.txt`。项目正在准备申请 SignPath Foundation 的免费开源代码签名服务，但尚未获批；当前安装器不能视为由 SignPath 签名。macOS 使用的 **Apple Developer ID Application** 证书只能签名 Apple 平台应用，不能用来签名 Windows `.exe`。详见[代码签名政策](CODE_SIGNING_POLICY.md)。
...
- 当前源码版本为 v0.4.2；macOS 现有 v0.4.1 正式包，Windows 为 v0.4.2 Preview；尚未提供 Android 或 iPhone 版本。
...
- Windows Native WLAN API 不提供可靠的噪声、SNR 和路由器侧 CCA；系统能力缺失时明确标为“不支持”，其他采集失败标为“未检测”，不会用推算值伪装成真实采集。
...
安装包的签名身份、构建来源、审批原则和校验方式见 [CODE_SIGNING_POLICY.md](CODE_SIGNING_POLICY.md)。macOS v0.4.1 已使用 Apple Developer ID 签名并完成公证；Windows v0.4.2 安装器目前未签名。项目正在准备申请 SignPath Foundation 免费开源代码签名服务，尚未获得批准或证书，因此不会宣称当前 Windows 安装包由 SignPath 签名。下载发布包时应只使用本仓库的正式 Release，并核对发布页提供的 SHA-256 校验值。
