...
> 当前统一产品版本：v0.4.0 · Windows 10 / 11 64 位 · x64 / ARM64 · 图形化 EXE 安装器
...
| 设备 | 安装器 |
| --- | --- |
| 普通 Intel / AMD Windows 电脑 | [WiFi-Health-Console-Setup-x64.exe](https://github.com/meyaomiao/wifi-health-console/releases/download/v0.4.0/WiFi-Health-Console-Setup-x64.exe) |
| Windows on ARM 设备 | [WiFi-Health-Console-Setup-arm64.exe](https://github.com/meyaomiao/wifi-health-console/releases/download/v0.4.0/WiFi-Health-Console-Setup-arm64.exe) |
...
Windows Preview 如果尚未配置 Windows Authenticode 代码签名证书，SmartScreen 可能提示“Windows 已保护你的电脑”或“未知发布者”。请先确认文件来自本项目的官方 [v0.4.0 统一 Release](https://github.com/meyaomiao/wifi-health-console/releases/tag/v0.4.0)，并对照发布页中的 `SHA256SUMS.txt`。

macOS 安装包使用的 **Apple Developer ID Application** 证书只能签名 Apple 平台应用，不能签名 Windows `.exe`，也不能转换成 Windows 代码签名证书。

## 已实现功能

| 页面 | Windows Preview v0.4.0 的实际行为 |
| --- | --- |
