...
> 当前 Windows 版本：v0.4.2 Preview · Windows 10 / 11 64 位 · x64 / ARM64 · 图形化 EXE 安装器

项目以 [MIT License](../LICENSE) 开源。依赖和随安装包分发内容的许可信息见[第三方许可声明](../THIRD-PARTY-NOTICES.md)，发布与签名规则见[代码签名政策](../CODE_SIGNING_POLICY.md)。
...
| 设备 | 安装器 |
| --- | --- |
| 普通 Intel / AMD Windows 电脑 | [WiFi-Health-Console-Setup-x64.exe](https://github.com/meyaomiao/wifi-health-console/releases/download/v0.4.2/WiFi-Health-Console-Setup-x64.exe) |
| Windows on ARM 设备 | [WiFi-Health-Console-Setup-arm64.exe](https://github.com/meyaomiao/wifi-health-console/releases/download/v0.4.2/WiFi-Health-Console-Setup-arm64.exe) |
...
当前 v0.4.2 Windows Preview 安装器尚未进行 Authenticode 签名，SmartScreen 可能提示“Windows 已保护你的电脑”或“未知发布者”。请先确认文件来自本项目的官方 [v0.4.2 Windows Release](https://github.com/meyaomiao/wifi-health-console/releases/tag/v0.4.2)，并对照发布页中的 `SHA256SUMS-windows.txt`。项目正在准备申请 SignPath Foundation 免费开源代码签名服务，但尚未获批；当前安装器不能视为由 SignPath 签名。

macOS 安装包使用的 **Apple Developer ID Application** 证书只能签名 Apple 平台应用，不能签名 Windows `.exe`，也不能转换成 Windows 代码签名证书。

## 已实现功能

| 页面 | Windows Preview v0.4.2 的实际行为 |
| --- | --- |
...
- Windows 公开 WLAN API 不提供可靠的噪声、SNR 和路由器侧 CCA。系统能力缺失时页面会明确显示“不支持”，其他采集失败显示“未检测”，不会伪造数值。
...
1. 使用 `global.json` 固定的 .NET SDK 10.0.302 恢复依赖并运行测试。
2. 发布 `win-x64` 和 `win-arm64` 自包含、多文件、未裁剪的应用。
3. 安装固定版本 NSIS 3.12.0 并生成两个 EXE 安装器。
...
当前 v0.4.2 Windows Release 中的安装器未签名。项目正在准备申请 SignPath Foundation 免费开源代码签名服务，尚未获得批准、项目配置或证书；在审批和受保护的签名流程真正上线前，不会把任何产物标记为 SignPath 已签名。签名身份、人工审批、发布来源和校验要求见[代码签名政策](../CODE_SIGNING_POLICY.md)。

现有 CI 在未配置签名时仍会生成可测试的未签名安装器。仓库也保留了使用自有 Authenticode PFX 的可选流程，需要由仓库所有者在 GitHub Actions secrets 中配置：

- `WINDOWS_SIGNING_CERTIFICATE_BASE64`：Windows 代码签名 PFX 文件的 Base64 内容。
- `WINDOWS_SIGNING_CERTIFICATE_PASSWORD`：该 PFX 的密码。

必须使用支持 Windows Authenticode Code Signing 的证书。macOS 使用的 **Apple Developer ID Application** 证书不能签名 Windows `.exe`，也不能转换成 Windows 代码签名证书。证书、私钥和密码只能保存在受保护的密钥服务或 GitHub Actions secrets 中，不能提交到仓库、Pull Request、Actions artifact 或 Issue。若产物未签名，Windows SmartScreen 可能在首次下载时显示“未知发布者”。
...
以下命令只面向从源码构建的开发者；普通用户不需要执行。为了与可验证发布环境一致，需要 Windows、`global.json` 指定的 .NET SDK 10.0.302 和 NSIS 3.12.0。先发布两个架构：
...
本地发布目录和安装包应包含根许可证、第三方许可声明及 `licenses/` 中的适用许可证文本。新增或升级 NuGet 包、原生运行时、字体、安装工具时，必须确认其采用 OSI 认可的许可证，并同步更新 [THIRD-PARTY-NOTICES.md](../THIRD-PARTY-NOTICES.md)。
