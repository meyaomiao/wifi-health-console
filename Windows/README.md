...
当前 v0.4.1 Windows Preview 安装器尚未进行 Authenticode 签名，SmartScreen 可能提示“Windows 已保护你的电脑”或“未知发布者”。请先确认文件来自本项目的官方 [v0.4.1 统一 Release](https://github.com/meyaomiao/wifi-health-console/releases/tag/v0.4.1)，并对照发布页中的 `SHA256SUMS.txt`。项目正在准备申请 SignPath Foundation 免费开源代码签名服务，但尚未获批；当前安装器不能视为由 SignPath 签名。
...
当前 v0.4.1 正式 Release 中的 Windows 安装器未签名。项目正在准备申请 SignPath Foundation 免费开源代码签名服务，尚未获得批准、项目配置或证书；在审批和受保护的签名流程真正上线前，不会把任何产物标记为 SignPath 已签名。签名身份、人工审批、发布来源和校验要求见[代码签名政策](../CODE_SIGNING_POLICY.md)。
